...
public class WrapperMenu extends FHCraftingMenu<WrapperBlockEntity> {

    private final ContainerData data;

    public WrapperMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(3));
    }

    public WrapperMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.WRAPPER_MENU.get(), pContainerId, ModBlocks.WRAPPER.get());
        blockEntity = ((WrapperBlockEntity) entity);
